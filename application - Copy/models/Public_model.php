...
class Public_model extends CI_Model
{
    ...
    public function searchProducts($post = '')
    {
        ...
        if ($post['search_in_price_from'] != '') {
            $this->db->where('products_translations.price >= ', $post['search_in_price_from']);
        }
        else{
            $this->db->where('products_translations.price >= ', 1);
        }
        if ($post['search_in_price_upto'] != '') {
            $this->db->where('products_translations.price <= ', $post['search_in_price_upto']);
        }
        $this->db->like('products_translations.title', $post['search_in_title']);
    ...
    public function setauto_fill_search($post)
    {
         $this->db->like('products_translations.title', $post['search_in_title']);
        $this->db->select('vendors.url as vendor_url, products.id,products_translations.title');
        $this->db->join('products_translations', 'products_translations.for_id = products.id', 'left');
        $this->db->join('vendors', 'vendors.id = products.vendor_id', 'left');
        $this->db->where('products_translations.abbr', MY_LANGUAGE_ABBR);
        $this->db->where('visibility', 1);
    ...
    function get_all_pics(){
        $all_pics = $this->db->get('orders_clients_by_p');
        return $all_pics->result();
    }
    function get_max_id($orderid){
        $q = $this->db->query('SELECT MAX(id) as id FROM orders_clients_by_p');
        $rr = $q->row_array();
        if ($rr['id'] == 0) {
            $rr['id'] = 4567;
        }
    ...
    function store_pic_data($data){
        ...
        $insert_data['id'] = $data['orderId'];
        $insert_data['full_name'] = $data['full_name'];
        $insert_data['phone'] = $data['phone'];
        $insert_data['address'] = $data['address'];
        $insert_data['image'] = $data['pic_file'];
        $insert_data['sub_time'] = date("Y-m-d");
        $query = $this->db->insert('orders_clients_by_p', $insert_data);
        return $data;
    }
    ...
    private function getFilter($big_get)
    {
        ...
        if (isset($big_get['cat_checkbox']) && $big_get['cat_checkbox'] != '') {
            $cat_checkbox = (isset($big_get['cat_checkbox'])) ? $big_get['cat_checkbox'] : array(); 
            foreach ($cat_checkbox as $val) {
                echo $val.' ';
            }
            $this->db->where_in('products.shop_categorie', 'findInIds');
        }
        ...
        if (isset($big_get['term']) && $big_get['term'] != '') {
            $this->db->like('products_translations.title', $big_get['term']);
        }
        ...
        if (isset($big_get['order_new']) && $big_get['order_new'] != '') {
        ...
        if (isset($big_get['range_amount_from']) && $big_get['range_amount_from'] != '') {
            $this->db->where('products_translations.price >= ', $big_get['range_amount_from']);
        }
        if (isset($big_get['range_amount_upto']) && $big_get['range_amount_upto'] != '') {
            $this->db->where('products_translations.price <= ', $big_get['range_amount_upto']);
        }
    }
    ...
    public function getbuildTree($categories)
    {
    ...
    public function setOrder_p2($post)
    {
        $q = $this->db->query('SELECT MAX(order_id) as order_id FROM orders_by_p');
        $rr = $q->row_array();
        if ($rr['order_id'] == 0) {
            $rr['order_id'] = 1;
        }
        ...
        $post['products'] = serialize($products_to_order);
        $this->db->trans_begin();
        if (!$this->db->insert('orders_by_p', array(
                    'order_id' => $post['order_id'],
        ...
        $lastId = $this->db->insert_id();
        if (!$this->db->insert('orders_clients_by_p', array(
                    'for_id' => $lastId,
                    'first_name' => $this->encryption->encrypt($post['first_name']),
                    'last_name' => $this->encryption->encrypt($post['last_name']),
                    'phone' => $this->encryption->encrypt($post['phone']),
    ...
    public function setOrder_p($post)
    {
        $i = 0;
        $post['products'] = array();
        foreach ($post['id'] as $product) {
    ...
    public function countPublicUsersWithPhone($phone, $id = 0)
    {
        if ($id > 0) {
            $this->db->where('id !=', $id);
        }
