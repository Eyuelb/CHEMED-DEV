...
$(".changeOrder2").change(function () {
    window.location.href = urls2.ordersOrderBy2 + $(this).val();
});
...
function changeOrdersOrderStatus(id, to_status, products, userEmail) {
    $.post(urls2.changeOrdersOrderStatus1, {the_id: id, to_status: to_status, products: products, userEmail: userEmail}, function (data) {
        if (data == 1) {
            if (to_status == 0) {
...
function changeOrdersOrderStatus2(id, to_status, products, userEmail) {
    $.post(urls.changeOrdersOrderStatus, {the_id2: id, to_status2: to_status, products: products, userEmail: userEmail}, function (data) {
        if (data == '1') {
            if (to_status == 0) {
                $('[data-action-id="' + id + '"] div.status b').text('No processed');
                $('[data-action-id="' + id + '"]').removeClass().addClass('bg-danger text-center');
...
function changeProductStatus1(id) {
...
function changeProductStatus2(id) {
    var to_status = $("#to-status").val();
    $.ajax({
        type: "POST",
        url: urls.productStatusChange,
        data: {id: id, to_status2: to_status}
    }).done(function (data) {
        if (data == '1') {
            if (to_status == 1) {
                $('[data-article-id="' + id + '"] .staus-is').text('Visible');
